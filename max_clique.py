...
def test(folder):
    ...
    src_pcd_path = folder + '/fragment_000.ply'
    tgt_pcd_path = folder + '/fragment_002.ply'
...
def infer(src_pcd, tgt_pcd, voxel_size):
    try:
        time001 = time.time()
        GTmat_path = '/Users/cucu/Desktop/Xcode/reg4recon/test/GTmat.txt' # ground truth transformation for calculate RE TE
        GTmat = np.loadtxt(GTmat_path, dtype=np.float32)
        ...
        src_kpts = src_pcd.voxel_down_sample(voxel_size) # default = 0.2
        tgt_kpts = tgt_pcd.voxel_down_sample(voxel_size) # default = 0.2

        # src_desc = extract_fpfh_features(src_kpts, 0.05)
        # tgt_desc = extract_fpfh_features(tgt_kpts, 0.05)

        src_desc = extract_fpfh_features(src_kpts, voxel_size) # default = 0.05
        tgt_desc = extract_fpfh_features(tgt_kpts, voxel_size) # default = 0.05
        ...
        result_transformation = final_trans         # 结果1

        result_information = o3d.pipelines.registration.get_information_matrix_from_point_clouds(
            src_pcd, tgt_pcd, 0.05 * 1.4, result_transformation)   # 结果2
        if result_information[5, 5] / min(len(src_pcd.points), len(tgt_pcd.points)) < 0.3:
            return (False, np.identity(4), np.zeros((6, 6)))

        time002 = time.time()
        print("+++++++++++++++++++++++++++++++++++++++++++++++++")
        print(time002 - time001)
        print("+++++++++++++++++++++++++++++++++++++++++++++++++")

        return (True, result_transformation, result_information)
...
if __name__ == '__main__':
    # test('test')
    (success, result_transformation, result_information) = infer(src_pcd=None, tgt_pcd=None)
